...
class DrawerFragment : Fragment(R.layout.fragment_drawer) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = AppDrawerAdapter(
            mutableListOf(),
            style,
            onClick = { pkg -> AppLauncher.open(requireContext(), pkg) },
            onLongClick = { entry -> showContextMenu(entry) }
        )
    ...
    private fun s(key: String) = Strings.get(requireContext(), key)

    private fun showContextMenu(entry: AppEntry) {
        val context = requireContext()
        val options = arrayOf(s("context_rename"), s("context_hide"), s("context_uninstall"))
    ...
    private fun showRenameDialog(entry: AppEntry) {
        ...
        val input = EditText(context).apply {
            setText(Prefs.getRename(context, entry.packageName) ?: entry.label)
        ...
        AlertDialog.Builder(context, R.style.Theme_MinimalLauncher_Dialog)
            ...
            .setPositiveButton(s("modes_save")) { _, _ ->
                ...
                Prefs.setRename(context, entry.packageName, if (newName == entry.label) null else newName)
                refreshList()
            }
            .setNeutralButton(s("rename_reset")) { _, _ ->
                Prefs.setRename(context, entry.packageName, null)
                refreshList()
            }
    ...
    private fun hideApp(entry: AppEntry) {
        val context = requireContext()
        val hidden = Prefs.getHiddenPackages(context).toMutableSet()
        hidden.add(entry.packageName)
        Prefs.setHiddenPackages(context, hidden)
        refreshList()
    }
    ...
    private fun confirmUninstall(entry: AppEntry) {
        val context = requireContext()
        AlertDialog.Builder(context, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(s("context_uninstall_title"))
            .setMessage(entry.label)
            .setPositiveButton(s("notes_delete_yes")) { _, _ ->
                try {
                    val uri = android.net.Uri.fromParts("package", entry.packageName, null)
                    ...
                    android.widget.Toast.makeText(
                        context, e.message ?: "Deinstallieren nicht möglich", android.widget.Toast.LENGTH_LONG
                    ).show()
                }
            }
            .setNegativeButton(s("notes_delete_no"), null)
            .show()
    }
    ...
    // Heftet den Zielbuchstaben normalerweise oben an. Wuerde das aber
    // Leerraum am unteren Ende erzeugen (weil danach nicht mehr genug Apps
    // folgen), wird exakt um den entstandenen Leerraum zurueckkorrigiert -
    // per echter Nachmessung nach dem Layout, nicht per Schaetzung. So ist
    // die letzte App danach garantiert voll sichtbar, kein manuelles
    // Nachscrollen mehr noetig.
    private fun scrollToLetterPosition(position: Int, totalItems: Int) {
        layoutManager.scrollToPositionWithOffset(position, 0)
        recyclerView.post {
            recyclerView.post {
                val lastVisiblePos = layoutManager.findLastVisibleItemPosition()
                if (lastVisiblePos == totalItems - 1) {
                    val lastChild = layoutManager.findViewByPosition(lastVisiblePos)
                    if (lastChild != null) {
                        val gap = recyclerView.height - lastChild.bottom
                        if (gap > 0) recyclerView.scrollBy(0, gap)
                    }
                }
                recyclerView.isVerticalFadingEdgeEnabled = recyclerView.canScrollVertically(1)
            }
        }
    }
