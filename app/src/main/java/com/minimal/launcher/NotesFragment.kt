...
class NotesFragment : Fragment(R.layout.fragment_notes) {
    ...
    // Mehrzeilen-Auswahl (langes Druecken auf eine Zeile startet sie):
    // die Toolbar wechselt dann von Bold/Bullet/Check auf Copy/Cancel.
    private var selectionMode = false
    private val selectedPositions = mutableSetOf<Int>()
    private lateinit var btnBold: View
    private lateinit var btnBullet: View
    private lateinit var btnCheck: View
    private lateinit var btnDeleteNote: View
    private lateinit var btnCopySelection: TextView
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = NoteLineAdapter(
            lines,
            typeface,
            onChanged = { saveNotes(); ensureLineVisible(focusedPosition) },
            ...
            onLongPressLine = { pos -> enterSelectionMode(pos) },
            onToggleSelect = { pos -> toggleSelect(pos) }
        )
        ...
        recyclerView.setOnClickListener { if (!selectionMode) focusLine(lines.size - 1, toEnd = true) }

        setupKeyboardVisibilityListener(view)

        btnBold = view.findViewById(R.id.btnBold)
        btnBullet = view.findViewById(R.id.btnBullet)
        ...
        btnBold.setOnClickListener { toggleType(NoteLineType.BOLD) }
        btnBullet.setOnClickListener { toggleType(NoteLineType.BULLET) }
        btnCheck.setOnClickListener { toggleType(NoteLineType.CHECK_OFF) }
        btnDeleteNote.setOnClickListener { confirmDeleteAll() }
        btnCopySelection.text = Strings.get(context, "notes_copy")
        btnCancelSelection.text = Strings.get(context, "notes_cancel")
        btnCopySelection.setOnClickListener { copySelection() }
    ...
    private fun updateSelectionToolbar() {
        val visibleWhenEditing = if (selectionMode) View.GONE else View.VISIBLE
        val visibleWhenSelecting = if (selectionMode) View.VISIBLE else View.GONE
        btnBold.visibility = visibleWhenEditing
        btnBullet.visibility = visibleWhenEditing
        btnCheck.visibility = visibleWhenEditing
        btnDeleteNote.visibility = visibleWhenEditing
        btnCopySelection.visibility = visibleWhenSelecting
        btnCancelSelection.visibility = visibleWhenSelecting
        if (selectionMode) {
            btnCopySelection.text = "${Strings.get(requireContext(), "notes_copy")} (${selectedPositions.size})"
        }
    }

    // Baut den kopierten Text aus den markierten Zeilen (in urspruenglicher
    // Reihenfolge), mit Praefix je Zeilentyp - so bleibt beim Einfuegen in
    // einen Chat/Dokument erkennbar, was Bullet/Haken war.
    private fun copySelection() {
        val context = requireContext()
        val selectedText = selectedPositions.sorted().joinToString("\n") { pos ->
            val line = lines.getOrNull(pos)
            if (line == null) "" else {
                val prefix = when (line.type) {
                    NoteLineType.BULLET -> "• "
                    NoteLineType.CHECK_OFF -> "☐ "
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("note", selectedText))
        Toast.makeText(context, Strings.get(context, "notes_copied_toast"), Toast.LENGTH_SHORT).show()
        exitSelectionMode()
    }
    ...
    private var pendingVisibilityCheck: Runnable? = null

    // Pragmatischer Ansatz: statt auf einen exakten Layout-Callback zu
    // vertrauen (der bei schnell wachsendem Text offenbar noch zu frueh
    // feuert), wird kurz gewartet, bis sich der Zeilenumbruch sicher gelegt
    // hat, und dann grosszuegig Abstand zur Tastatur eingeplant.
    private fun ensureLineVisible(position: Int) {
        val extraBuffer = (32 * resources.displayMetrics.density).toInt()
        pendingVisibilityCheck?.let { visibilityHandler.removeCallbacks(it) }
        val check = Runnable {
            val child = layoutManager.findViewByPosition(position)
            val visibleBottom = recyclerView.height - recyclerView.paddingBottom - extraBuffer
            if (child != null) {
                when {
        ...
        visibilityHandler.postDelayed(check, 60)
    }
