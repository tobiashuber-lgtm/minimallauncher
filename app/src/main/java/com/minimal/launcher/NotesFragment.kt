...
class NotesFragment : Fragment(R.layout.fragment_notes) {
    ...
    // Erkennt Tastatur auf/zu ueber die Fensterhoehe (kein direktes System-
    // Ereignis dafuer verfuegbar) und scrollt beim Schliessen automatisch
    // zur letzten Zeile, damit dort kein Leerraum haengen bleibt.
    private fun setupKeyboardVisibilityListener(root: View) {
        ...
        root.viewTreeObserver.addOnGlobalLayoutListener {
            ...
            if (isKeyboardVisible) {
                recyclerView.post { recyclerView.scrollToPosition(focusedPosition) }
            } else if (wasKeyboardVisible) {
                recyclerView.post { recyclerView.scrollToPosition(lines.size - 1) }
            }
