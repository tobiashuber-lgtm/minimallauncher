...
class ModeManagerActivity : AppCompatActivity() {
    ...
    private fun buildList() {
        addHeader(s("modes_standard_header"))

        addRow("\"${Prefs.getStandardModeDisplayName(this)}\" ${s("modes_rename_standard")}") {
            showRenameStandardDialog()
        }

        addRow(s("modes_edit_standard")) {
            openModeEdit("Standard")
        }

        addRow(s("modes_create_new")) { showCreateDialog() }

        val modes = Prefs.getModeNames(this)
        if (modes.isNotEmpty()) {
            addHeader(s("modes_your_modes"))
            for (mode in modes) {
    ...
    private fun showCreateDialog() {
        ...
        AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(s("modes_new_name_title"))
            .setView(input)
            .setPositiveButton(s("modes_create_btn")) { _, _ ->
                val name = input.text.toString().trim()
