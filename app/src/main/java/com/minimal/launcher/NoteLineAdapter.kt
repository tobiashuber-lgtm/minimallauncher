...
class NoteLineAdapter(
    ...
    private val onSplitLine: (position: Int, textBefore: String, textAfter: String) -> Unit,
    private val onBackspaceAtStart: (position: Int) -> Boolean,
    private val isSelectionMode: () -> Boolean = { false },
    private val isSelected: (Int) -> Boolean = { false },
    private val onLongPressLine: (Int) -> Unit = {},
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        val etLine: NoteEditText = view.findViewById(R.id.etLine)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.etLine.onBackspaceAtStart = null
        ...
        val watcher = object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                if (newlineIndex >= 0) {
                    val before = text.substring(0, newlineIndex)
                    val after = text.substring(newlineIndex + 1)
                    onSplitLine(holder.bindingAdapterPosition, before, after)
                } else {
                    line.text = text
                    onChanged()
                }
        ...
        holder.etLine.onBackspaceAtStart = { onBackspaceAtStart(holder.bindingAdapterPosition) }

        // Mehrzeilen-Auswahl: lange druecken startet den Auswahlmodus,
        // waehrend die Auswahl laeuft schaltet ein Tap auf die Zeile
        // (statt sie zu bearbeiten) einfach die Markierung um.
        val selectionMode = isSelectionMode()
        val selected = isSelected(position)
        holder.itemView.setBackgroundColor(if (selected) 0xFF262626.toInt() else 0x00000000)
        holder.etLine.isFocusable = !selectionMode
        holder.etLine.isFocusableInTouchMode = !selectionMode
        holder.checkbox.isEnabled = !selectionMode

        val selectClick = View.OnClickListener {
            if (isSelectionMode()) onToggleSelect(holder.bindingAdapterPosition)
        }
        val longPress = View.OnLongClickListener {
            onLongPressLine(holder.bindingAdapterPosition)
            true
        }
        holder.itemView.isClickable = true
        holder.itemView.setOnClickListener(selectClick)
        holder.itemView.setOnLongClickListener(longPress)
        holder.etLine.setOnClickListener(selectClick)
        holder.etLine.setOnLongClickListener(longPress)
        holder.tvBullet.setOnClickListener(selectClick)
        holder.tvBullet.setOnLongClickListener(longPress)
    }
