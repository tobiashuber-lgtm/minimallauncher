...
class DockPickerActivity : AppCompatActivity() {
    ...
    private fun buildRows() {
        ...
        for (i in slotLabels.indices) {
            ...
            val currentLabel = apps.firstOrNull { it.packageName == currentPkg }?.label ?: "Standard"
    ...
    private fun showPicker(slot: Int) {
        val options = listOf("Standard (zurücksetzen)") + apps.map { it.label }
        AlertDialog.Builder(this)
            ...
            .setItems(options.toTypedArray()) { _, which ->
                if (which == 0) {
                    ...
                    Prefs.setDockPackage(this, slot, apps[which - 1].packageName)
                }
                buildRows()
            }
            .show()
