package com.minimal.launcher

import android.content.Intent
import android.os.Bundle
import android.view.View
import android.widget.LinearLayout
import android.widget.TextView
import androidx.appcompat.app.AlertDialog
import androidx.appcompat.app.AppCompatActivity

class DockPickerActivity : AppCompatActivity() {

    private val slotLabels = listOf("Slot 1 (links)", "Slot 2", "Slot 3", "Slot 4 (rechts)")
    private lateinit var apps: List<AppEntry>
    private lateinit var container: LinearLayout

    private fun s(key: String) = Strings.get(this, key)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_dock_picker)
        container = findViewById(R.id.dockSlotsContainer)

        val hidden = Prefs.getHiddenPackages(this)
        apps = loadApps().filter { it.packageName !in hidden }

        buildRows()
    }

    private fun buildRows() {
        container.removeAllViews()
        for (i in slotLabels.indices) {
            val currentPkg = Prefs.getDockPackage(this, i)
            val iconKey = Prefs.getDockIcon(this, i)
            val currentLabel = when (currentPkg) {
                null -> s("dock_default")
                "__EMPTY__" -> s("dock_empty")
                else -> apps.firstOrNull { it.packageName == currentPkg }?.label ?: s("dock_default")
            }
            val iconSuffix = if (iconKey != null) " · Icon: ${DockIcons.labelFor(iconKey)}" else ""

            val row = TextView(this).apply {
                text = "${slotLabels[i]}: $currentLabel$iconSuffix"
                setTextColor(0xFFFFFFFF.toInt())
                textSize = 15f
                typeface = android.graphics.Typeface.MONOSPACE
                setPadding(0, dp(14), 0, dp(14))
                setOnClickListener { showPicker(i) }
            }
            container.addView(row)

            val divider = View(this).apply {
                layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(1) / 2)
                setBackgroundColor(0xFF2A2A2A.toInt())
            }
            container.addView(divider)
        }
    }

    private fun showPicker(slot: Int) {
        val options = listOf(s("dock_default_reset"), s("dock_empty_option")) + apps.map { it.label }
        AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(slotLabels[slot])
            .setItems(options.toTypedArray()) { _, which ->
                when (which) {
                    0 -> {
                        Prefs.setDockPackage(this, slot, null)
                        Prefs.setDockIcon(this, slot, null)
                        buildRows()
                    }
                    1 -> {
                        Prefs.setDockPackage(this, slot, "__EMPTY__")
                        Prefs.setDockIcon(this, slot, null)
                        buildRows()
                    }
                    else -> {
                        Prefs.setDockPackage(this, slot, apps[which - 2].packageName)
                        showIconPicker(slot)
                    }
                }
            }
            .show()
    }

    private fun showIconPicker(slot: Int) {
        val options = listOf(s("dock_own_icon")) + DockIcons.options.map { it.second.first }
        AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(s("dock_choose_icon"))
            .setItems(options.toTypedArray()) { _, which ->
                if (which == 0) {
                    Prefs.setDockIcon(this, slot, null)
                } else {
                    Prefs.setDockIcon(this, slot, DockIcons.options[which - 1].first)
                }
                buildRows()
            }
            .show()
    }

    private fun loadApps(): List<AppEntry> {
        val pm = packageManager
        val intent = Intent(Intent.ACTION_MAIN, null).addCategory(Intent.CATEGORY_LAUNCHER)
        return pm.queryIntentActivities(intent, 0).map {
            AppEntry(it.loadLabel(pm).toString(), it.activityInfo.packageName)
        }.distinctBy { it.packageName }.sortedBy { it.label.lowercase() }
    }

    private fun dp(value: Int): Int = (value * resources.displayMetrics.density).toInt()
}
