...
class ModeNotificationMuteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.btnSelectAll).apply {
            ...
            setOnClickListener {
                ...
                Prefs.setModeMutedNotificationPackages(this@ModeNotificationMuteActivity, mode, muted)
    ...
    private fun updateHeader() {
        val access = if (NotificationAccessHelper.hasPermission(this)) "" else "\n⚠️ ${s("mute_access_missing")}"
        tvHeader.text = "${s("mute_count_prefix").format(mode)}${muted.size} ${s("mute_count_suffix")}$access"
