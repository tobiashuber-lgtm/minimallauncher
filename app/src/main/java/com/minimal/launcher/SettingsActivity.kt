...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun buildSettingsList() {
        ...
        addButtonRow(s("row_timelimit_durations")) {
            startActivity(Intent(this, TimeLimitDurationsActivity::class.java))
        }
        ...
        addSectionHeader(s("section_weather"))

        addToggleRow(s("row_weather_show"), Prefs.getWeatherEnabled(this)) {
            Prefs.setWeatherEnabled(this, it)
        }

        addToggleRow(s("row_weather_gps"), Prefs.getWeatherUseGps(this)) { enabled ->
            if (enabled) requestLocationPermissionIfNeeded()
            Prefs.setWeatherUseGps(this, enabled)
        }

        val locationLabel = if (Prefs.getWeatherUseGps(this)) {
            s("weather_current_location")
        } else {
            Prefs.getWeatherLocationName(this).ifBlank { s("row_weather_location") }
        }
        addButtonRow("${s("row_weather_location")}: $locationLabel") {
            showLocationDialog()
        }

        addSliderRow(s("row_weather_size"), 10, 22, Prefs.getWeatherSizeSp(this)) {
            Prefs.setWeatherSizeSp(this, it)
        }

        val customLink = Prefs.getWeatherCustomLink(this)
        addButtonRow("${s("row_weather_custom_link")}: ${customLink.ifBlank { "Google" }}") {
        ...
        addButtonRow(s("row_setup_wizard")) {
    ...
    private fun addFooter() {
        val text = android.text.SpannableString("GHOSTS Launcher\nby ghostsmakemusic ↗")
        val start = text.indexOf("ghostsmakemusic")
        val end = start + "ghostsmakemusic".length
        text.setSpan(android.text.style.UnderlineSpan(), start, end, android.text.Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        ...
        val tv = TextView(this).apply {
            ...
            setOnClickListener {
                startActivity(Intent(Intent.ACTION_VIEW, android.net.Uri.parse("https://linktr.ee/ghostsmakemusic")))
            }
        }
        container.addView(tv)
    }

    private fun showCustomLinkDialog() {
        val input = android.widget.EditText(this).apply {
            hint = s("weather_custom_link_dialog_title")
            setText(Prefs.getWeatherCustomLink(this@SettingsActivity))
        }
        androidx.appcompat.app.AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(s("row_weather_custom_link"))
            .setView(input)
            .setPositiveButton(s("modes_save")) { _, _ ->
                var link = input.text.toString().trim()
                if (link.isNotBlank() && !link.startsWith("http://") && !link.startsWith("https://")) {
                    link = "https://$link"
                }
                Prefs.setWeatherCustomLink(this, link)
                container.removeAllViews()
                buildSettingsList()
            }
    ...
    private fun showLocationDialog() {
        val input = android.widget.EditText(this).apply {
            hint = s("weather_location_dialog_title")
            setText(Prefs.getWeatherLocationName(this@SettingsActivity))
        }
        androidx.appcompat.app.AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle(s("weather_location_dialog_title"))
            .setView(input)
            .setPositiveButton(s("modes_save")) { _, _ ->
                val cityName = input.text.toString().trim()
                if (cityName.isNotEmpty()) {
                    WeatherHelper.geocodeCity(cityName) { result ->
                        if (result != null) {
                            Prefs.setWeatherLocationName(this, result.displayName)
                            Prefs.setWeatherCoords(this, result.lat, result.lon)
                            Prefs.setWeatherUseGps(this, false)
                        } else {
                            android.widget.Toast.makeText(this, s("weather_location_not_found"), android.widget.Toast.LENGTH_SHORT).show()
                        }
                        container.removeAllViews()
                        buildSettingsList()
                    }
