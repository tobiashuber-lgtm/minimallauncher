...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun buildSettingsList() {
        ...
        val modeOptions = listOf("Standard") + Prefs.getModeNames(this)
        addCycleRow(
            "Aktiver Modus",
            modeOptions,
            modeOptions,
        ...
        addSectionHeader("Gesten")

        addButtonRow("Swipe nach oben: ${swipeActionLabel(Prefs.getSwipeUpAction(this))}") {
            showSwipeActionPicker { Prefs.setSwipeUpAction(this, it) }
        }

        addButtonRow("Swipe nach unten: ${swipeActionLabel(Prefs.getSwipeDownAction(this))}") {
            showSwipeActionPicker { Prefs.setSwipeDownAction(this, it) }
        }
    ...
    private fun swipeActionLabel(action: String): String = when {
        action == "none" -> "Keine"
        action == "flashlight" -> "Taschenlampe"
        action == "notes" -> "Notizen öffnen"
        action == "drawer" -> "App-Drawer öffnen"
    ...
    private fun showSwipeActionPicker(onChosen: (String) -> Unit) {
        val fixedOptions = listOf(
            "none" to "Keine",
            "flashlight" to "Taschenlampe",
            "notes" to "Notizen öffnen",
            "drawer" to "App-Drawer öffnen"
        )
        val options = fixedOptions.map { it.second } + "Bestimmte App wählen..."
        androidx.appcompat.app.AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            .setTitle("Aktion wählen")
            .setItems(options.toTypedArray()) { _, which ->
                if (which < fixedOptions.size) {
                    onChosen(fixedOptions[which].first)
                    recreate()
                } else {
                    showAppPickerForSwipe(onChosen)
                }
            }
            .show()
    }

    private fun showAppPickerForSwipe(onChosen: (String) -> Unit) {
        ...
        androidx.appcompat.app.AlertDialog.Builder(this, R.style.Theme_MinimalLauncher_Dialog)
            ...
            .setItems(apps.map { it.first }.toTypedArray()) { _, which ->
                onChosen("app:${apps[which].second}")
                recreate()
            }
            .show()
    }
