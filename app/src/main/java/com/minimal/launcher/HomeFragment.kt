...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun showModeSwitcher() {
        ...
        val modes = listOf("Standard") + Prefs.getModeNames(context)
        ...
        androidx.appcompat.app.AlertDialog.Builder(context)
            ...
            .setSingleChoiceItems(modes.toTypedArray(), currentIndex) { dialog, which ->
                Prefs.setCurrentMode(context, modes[which])
                dialog.dismiss()
                applyPaletteAndFonts()
                populateFavorites()
                updateModeIndicator()
    ...
    private fun updateModeIndicator() {
        val mode = Prefs.getCurrentMode(requireContext())
        val tvMode = rootView.findViewById<TextView>(R.id.tvModeIndicator)
        tvMode.text = mode
        tvMode.visibility = if (mode == "Standard") View.GONE else View.VISIBLE
    }
            ...
            if (assignedPkg != null) {
                try {
                    ...
                    imageView.setOnClickListener {
                        AppLauncher.open(context, assignedPkg)
                    }
