...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        tvDate.setOnClickListener { performDateAction(Prefs.getDateClickAction(requireContext())) }
        tvDate.setOnLongClickListener { performDateAction(Prefs.getDateLongClickAction(requireContext())); true }
    ...
    private fun performDateAction(action: String) {
        when (action) {
            "calendar" -> openCalendar()
            "new_event" -> openNewCalendarEvent()
            else -> { /* "none": bewusst keine Aktion */ }
        }
    }
