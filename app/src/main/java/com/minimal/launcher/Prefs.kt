...
object Prefs {
    ...
    fun getStatusBarHidden(context: Context): Boolean =
        prefs(context).getBoolean("statusbar_hidden", false)
    ...
    // --- Zeitlimit-Dauern: standardmaessig 5/10/15 Minuten, individuell
    // in den Einstellungen aenderbar ---

    fun getTimeLimitOptions(context: Context): List<Int> = listOf(
        prefs(context).getInt("timelimit_option_1", 5),
        prefs(context).getInt("timelimit_option_2", 10),
        prefs(context).getInt("timelimit_option_3", 15)
    )

    fun setTimeLimitOption(context: Context, index: Int, minutes: Int) {
        prefs(context).edit().putInt("timelimit_option_${index + 1}", minutes).apply()
    }
    ...
    fun getAppLanguage(context: Context): String =
        prefs(context).getString("app_language", "en") ?: "en"
    ...
    // --- Einrichtungs-Assistent: automatisch beim ersten Start zeigen ---

    fun hasSeenSetup(context: Context): Boolean =
        prefs(context).getBoolean("has_seen_setup", false)

    fun setSeenSetup(context: Context, value: Boolean) {
        prefs(context).edit().putBoolean("has_seen_setup", value).apply()
    }

    // --- Wetter (optional, standardmaessig aus) ---

    fun getWeatherEnabled(context: Context): Boolean =
        prefs(context).getBoolean("weather_enabled", false)

    fun setWeatherEnabled(context: Context, value: Boolean) {
        prefs(context).edit().putBoolean("weather_enabled", value).apply()
    }

    fun getWeatherUseGps(context: Context): Boolean =
        prefs(context).getBoolean("weather_use_gps", false)

    fun setWeatherUseGps(context: Context, value: Boolean) {
        prefs(context).edit().putBoolean("weather_use_gps", value).apply()
    }

    fun getWeatherLocationName(context: Context): String =
        prefs(context).getString("weather_location_name", "") ?: ""

    fun setWeatherLocationName(context: Context, value: String) {
        prefs(context).edit().putString("weather_location_name", value).apply()
    }

    fun getWeatherLat(context: Context): Float = prefs(context).getFloat("weather_lat", 0f)
    fun getWeatherLon(context: Context): Float = prefs(context).getFloat("weather_lon", 0f)

    fun setWeatherCoords(context: Context, lat: Float, lon: Float) {
        prefs(context).edit().putFloat("weather_lat", lat).putFloat("weather_lon", lon).apply()
    }

    fun getWeatherSizeSp(context: Context): Int = prefs(context).getInt("weather_size_sp", 13)
    fun setWeatherSizeSp(context: Context, value: Int) {
        prefs(context).edit().putInt("weather_size_sp", value).apply()
    }

    // Kurzzeit-Cache, damit nicht bei jedem Home-Screen-Aufruf neu
    // angefragt wird - nur wenn der Cache aelter als 30 Minuten ist.
    fun getWeatherCache(context: Context): Triple<Int, Int, Long> = Triple(
        prefs(context).getInt("weather_cache_temp", 0),
        prefs(context).getInt("weather_cache_code", -1),
        prefs(context).getLong("weather_cache_time", 0L)
    )

    fun setWeatherCache(context: Context, temp: Int, code: Int) {
        prefs(context).edit()
            .putInt("weather_cache_temp", temp)
            .putInt("weather_cache_code", code)
            .putLong("weather_cache_time", System.currentTimeMillis())
            .apply()
    }

    // Eigener Link statt der Google-Standardsuche - leer = Standard verwenden.
    fun getWeatherCustomLink(context: Context): String =
        prefs(context).getString("weather_custom_link", "") ?: ""

    fun setWeatherCustomLink(context: Context, value: String) {
        prefs(context).edit().putString("weather_custom_link", value).apply()
    }
}
