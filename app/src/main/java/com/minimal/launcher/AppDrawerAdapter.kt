...
class AppDrawerAdapter(
    ...
    // Wenn gesetzt: alles ausserhalb dieser Buchstaben-Gruppe wird gedimmt
    // (grau) dargestellt, waehrend an der Seitenleiste gezogen wird.
    var highlightLetter: Char? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val dimmed = highlightLetter != null && item.groupLetter != highlightLetter
        val dimColor = 0xFF454545.toInt()

        if (item.isHeader) {
            holder.tvName.text = item.label
            holder.tvName.typeface = style.typeface
            holder.tvName.setTextColor(if (dimmed) dimColor else style.secondaryColor)
        ...
        holder.tvName.text = item.label
        holder.tvName.typeface = style.typeface
        holder.tvName.setTextColor(if (dimmed) dimColor else style.textColor)
        ...
        holder.tvUsage.setTextColor(if (dimmed) dimColor else style.secondaryColor)
