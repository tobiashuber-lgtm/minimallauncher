...
## Update: Feinschliff Runde 6
- **App-Drawer**: dezenter Fade-Effekt am unteren (und oberen) Rand der
  Liste - die letzte sichtbare App verschwindet optisch etwas weicher
  statt abrupt am Bildschirmrand abzuschneiden
- **Notizen, Editor-Verhalten deutlich verbessert**:
  - Echter automatischer Wortumbruch beim Tippen (kein Enter mehr nötig,
    schreibt sich wie in einer normalen Chat-/Notiz-App weiter)
  - Enter teilt die Zeile an der Cursor-Position - bei Bullet-Punkt/
    Checkliste erbt die neue Zeile denselben Typ (Haken startet wieder
    unangehakt)
  - Backspace ganz am Zeilenanfang: bei Bullet/Checkliste wird nur die
    Formatierung entfernt (zurück zu normalem Text); bei normalem Text
    verschmilzt die Zeile mit der vorherigen (Cursor landet exakt an der
    Verschmelzungsstelle) - wie in einem klassischen Texteditor

## Update: Feinschliff Runde 7
- **App-Drawer Fade**: schaltet sich automatisch ab, sobald man ganz unten
  in der Liste angelangt ist - die letzte App wird nicht mehr überblendet
- **Notepad**:
  - Tippen irgendwo im schwarzen Bereich (nicht nur exakt auf eine Zeile)
    fokussiert die passende Zeile
  - Leeres Notepad: Tastatur öffnet sich automatisch beim Reinwischen,
    Cursor direkt in Zeile 1
  - Text verschwindet nicht mehr hinter der Tastatur - Bildschirm passt
    sich an (aktuelle Zeile bleibt sichtbar), beim Schließen der Tastatur
    rutscht die unterste Zeile automatisch ganz nach unten
  - Bullet-Punkt/Häkchen richten sich jetzt oben aus statt vertikal
    zentriert - bei mehrzeiligem Text wird dadurch nichts mehr am unteren
    Bildschirmrand abgeschnitten
  - Tastatur schließt sich automatisch, wenn man vom Notepad wegwischt

## Update: Feinschliff Runde 8
- **App-Drawer Kontextmenü**: langes Drücken auf eine App zeigt jetzt
  "Umbenennen" / "Ausblenden" / "Deinstallieren" (Space Mono, Schwarz/Weiß)
  - Umbenennen/Ausblenden greifen sofort, ohne extra in die Einstellungen
    zu müssen
  - Deinstallieren fragt erst "Wirklich deinstallieren?" (Ja/Nein), dann
    übernimmt Android selbst die finale Bestätigung (das ist normal so -
    eine App kann eine andere nicht ohne diese System-Bestätigung
    deinstallieren)
- **Drawer-Fade & Notepad-Tastatur**: Race-Condition-Fixes für präzisere
  Sichtbarkeits-Berechnung (siehe vorherige Einträge)

## Update: Feinschliff Runde 9
- **Footer-Link**: "GHOSTS Launcher" / "by ghostsmakemusic ↗" (unterstrichen,
  mit Pfeil), führt jetzt zu linktr.ee/ghostsmakemusic
- **Hintergrundfarbe**: reicht jetzt wirklich bis ganz an den Bildschirmrand
  (auch hinter Status-/Navigationsleiste) - vorher blitzte dort noch
  fest einprogrammiertes Schwarz durch
- **Deinstallieren im Drawer**: robuster gemacht (Fehlerabfangen +
  Rückmeldung, falls es doch nicht klappt)
- **Kamera-Dock-Icon**: öffnet jetzt die vollständige Kamera-App statt nur
  einen eingeschränkten "Foto aufnehmen und zurückgeben"-Modus

## Update: Feinschliff Runde 10
- **Status-/Navigationsleiste komplett transparent**: Hintergrundfarbe
  reicht jetzt wirklich bis in jede Ecke
- **Einrichtungs-Assistent (neu)**: erscheint automatisch beim allerersten
  Start, führt durch alle 4 nötigen Berechtigungen (Standard-
  Startbildschirm, Nutzungszugriff, Benachrichtigungszugriff, Anzeige über
  anderen Apps) mit Status-Anzeige (✓/→) - jederzeit über Einstellungen
  → "Einrichtungs-Assistent erneut öffnen" erreichbar. Wichtig: die
  eigentliche Bestätigung (Antippen des Schalters in den jeweiligen
  Android-Einstellungen) muss aus Sicherheitsgründen immer der Nutzer
  selbst machen - keine App darf das automatisch für sich freischalten,
  auch der eigene Launcher nicht.

## Update: Wetter (neu, optional)
- **Wetter-Anzeige** auf dem Home-Screen (klein, unter dem Akku-Balken):
  Temperatur + dezentes Mono-Icon passend zur Wetterlage (Sonne, Wolke,
  Regen, Schnee, Gewitter, Nebel)
- Standardmäßig **ausgeschaltet** - in Einstellungen → "Wetter" aktivierbar
- **Ort festlegen**: Stadt eintippen, wird automatisch in Koordinaten
  umgewandelt (Open-Meteo Geocoding, kostenlos, kein API-Key nötig)
- **GPS-Option**: automatische Standort-Aktualisierung statt festem Ort
  (standardmäßig aus, braucht einmalig die Standort-Berechtigung)
- **Schriftgröße** separat einstellbar
- Aktualisiert sich beim Öffnen des Home-Screens, aber nur wenn die
  letzten Daten älter als 30 Minuten sind (spart Anfragen) - bei GPS wird
  der Standort bei jedem Öffnen neu geprüft
- Datenquelle: [Open-Meteo](https://open-meteo.com) - komplett kostenlos,
  keine Anmeldung, kein API-Key

## Update: Statusleiste & Wetter-Feinschliff
- **Statusleiste ausblenden**: Standardwert jetzt "Aus" - beim Ausblenden
  zeigte manche Geräte einen systemeigenen (nicht beeinflussbaren)
  Wisch-Hinweisstreifen in Schwarz an dieser Stelle. Mit sichtbarer
  Statusleiste ist die Farbe garantiert korrekt. Die Option zum Ausblenden
  bleibt in den Einstellungen weiterhin verfügbar.
- **9 statt 6 Wetter-Icons**: Sonne, leicht bewölkt, bewölkt, Nebel,
  Nieselregen, Regen, Schauer, Schnee, Gewitter
- **Eigener Wetter-Link**: in Einstellungen → "Eigener Wetter-Link" kannst
  du die Google-Standardsuche durch eine beliebige eigene URL ersetzen
  (leer lassen = Google-Standard bleibt aktiv)

## Update: Zwei Bugfixes
- **Eigener Wetter-Link**: fehlendes `https://` wird jetzt automatisch
  ergänzt (ohne das schlug das Öffnen fehl und es fiel stumm auf Google
  zurück) - am besten den vollständigen Link inkl. deiner Stadt einfügen
  (z.B. `https://www.wetteronline.at/wetter/innsbruck`)
- **Einrichtungs-Assistent "Als Standard festlegen"**: robuster gemacht
  (Fehler werden abgefangen, es gibt immer eine sichtbare Reaktion -
  entweder den System-Dialog oder als letzten Fallback eine Meldung)

## Update: Notizen mehrzeilig markieren & Standard-Modus stummschalten
- **Notizen - mehrere Zeilen markieren & kopieren**: langes Drücken auf eine
  Zeile startet den Auswahlmodus (Zeile wird grau hinterlegt), danach durch
  Antippen weitere Zeilen dazu- oder abwählen. Die Werkzeugleiste wechselt
  währenddessen von Bold/Bullet/Haken zu "Kopieren (Anzahl)" / "Abbrechen".
  Kopieren fügt die markierten Zeilen in ursprünglicher Reihenfolge
  zusammen (mit "• "/"☐ "/"☑ " je nach Zeilentyp) und legt das Ergebnis in
  die Zwischenablage - danach ganz normal in eine Nachricht oder ein
  Dokument einfügen.
- **Benachrichtigungen stummschalten im Standard-Modus**: die Auswahl "Apps
  stummschalten" gibt es jetzt auch im Standard-Modus (vorher nur bei
  eigenen Modi verfügbar) - Zugriff über Modi → Standard bearbeiten.
